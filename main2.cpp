...
void test_clear ()
{
  ...
  for (int i = 0; i < 1024; i++) {
      assert(!h2.contains_key (i)); // All keys were deleted
  }
  h2.insert (1, 10);
  assert(h2.size () == 1);
  assert(h2.at (1) == 10);
...
void test_dictionary_erase ()
{
  ...
  assert(d1.size () == 12);
  assert(d1.erase (to_string (2)) == true); // Should return true upon success
  assert(d1.size () == 11);
  assert(d1.erase (to_string (3)) == true); // Should return true upon success
  assert(d1.size () == 10);
  assert(d1.erase (to_string (4)) == true); // Should return true upon success
  assert(d1.size () == 9);
  assert(d1.erase (to_string (5)) == true); // Should return true upon success
  assert(d1.size () == 8);
